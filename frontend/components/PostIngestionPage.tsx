...
export default function PostIngestionPage({ sessionId }: PostIngestionPageProps) {
  ...
  const handlePredictWarnings = async () => {
    ...
    // Simulate progress steps (optimistic UI)
    setTimeout(() => {
      setCurrentStep(1)
      setProgressMessage('Extracting clauses from documents...')
    }, 500)

    setTimeout(() => {
      setCompletedSteps(prev => new Set(prev).add(1))
      setCurrentStep(2)
      setProgressMessage('Researching legal amendments and updates...')
    }, 30000)

    setTimeout(() => {
      setCompletedSteps(prev => new Set(prev).add(2))
      setCurrentStep(3)
      setProgressMessage('Analyzing risks and generating report...')
    }, 60000)
